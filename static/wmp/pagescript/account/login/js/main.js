require(['jquery','modules/net/wAjax','wxapi','common/jquery-plugins/fastclick'],function($,wAjax,wx,fastclick){
	wx.config($.extend({
		debug: DEBUG,
		jsApiList: []
	},WXCONFIG));
	...
	var cancelBigPaddingId;
	$('input').on('focus',function(){
		clearTimeout(cancelBigPaddingId);
		$('.container').addClass('on-foucs-big-padding');
	}).on('blur',function(){
		cancelBigPaddingId = setTimeout(function(){
			$('.container').removeClass('on-foucs-big-padding');
		},100);
	});
